...
To define the BDM, click on *New* in the coolbar then select *Business Data Model*. The current BDM definition is displayed.
...
Two maven artifacts are generated from the Business Data Model : *<ProjectId>-bdm-model* and *<ProjectId>-bdm-dao-client*. +
The `version` and `groupId` of those artifacts matches the Bonita project version. 
You have the possibility to view and retrieve the `bdm-model` dependency from the BDM editor `Maven artifact properties` section. +
Those maven artifacts are meant to be used from REST API extensions, using the following dependencies:

.The generated bdm-model dependency template
[source,xml]
----
<dependency>
	<groupId>[PROJECT GROUP ID]</groupId>
	<artifactId>[PROJECT ID]-bdm-model</artifactId>
	<version>[PROJECT VERSION]</version>
	<scope>provided</scope>
</dependency>
----
Contains all Business Object entities and DAO interfaces.
Can be used as dependency of:

* The main application module
* Any Rest API Extension
* A Java clients using the https://github.com/bonitasoft/bonita-java-client#41-bdm-typed-queries[bonita-java-client] and the Bdm typed queries
* Integration tests, using the xref:test-toolkit:business-data.adoc#_as_a_typed_object[Bonita Test Toolkit] and the typed Business Objects.

.The generated bdm-dao-client dependency template
[source,xml]
----
<dependency>
    <groupId>[PROJECT GROUP ID]</groupId>
    <artifactId>[PROJECT ID]-bdm-dao-client</artifactId>
    <version>[PROJECT VERSION]</version>
    <scope>provided</scope>
</dependency>
----
Contains client side implementation of the DAO.
Can be used as dependency of a java clients using the https://search.maven.org/search?q=g:org.bonitasoft.engine%20AND%20a:bonita-client[bonita-client] and the `APIClient#getDAO` factory method.
...
== Build and Deploy BDM artifacts

It is possible to automate the build and deployment of the `*-bdm-model` and `*-bdm-dao-client` artifacts using Maven. Publishing those artifacts can have its purpose if you need to consume them in any other maven or gradle projects outside of the Bonita project.

It is required to have https://maven.apache.org/install.html[maven properly installed] for the following instructions. 

Open a terminal at the root of the Bonita project.

To install the dependencies in your local repository you can run:
...
To publish the dependencies on a remote repository (Nexus, Artifactory, Github packages...etc) you can run:

[source, shell]
----
$ mvn deploy -DaltDeploymentRepository=<server_id>::<remote_repository_url>
----
NOTE: Proper remote repository credentials must be configured in your maven settings (`<USER_HOME>/.m2/settings.xml`) for the given `server_id` if required.

By default running those commands from the project root will build and deploy all the project submodules. You may cherry pick what submodule to deploy using:

.Only publish the `*-bdm-model` artifact
[source, shell]
----
$ mvn -pl bdm/model -am deploy -DaltDeploymentRepository=<server_id>::<remote_repository_url> <1>
----
<1> `-pl` let you define a specific projects (modules) list. `bdm/model` is the path in the project to the `*-bdm-model` module. `-am` force the build of the dependent project.

.Only publish the `*-bdm-dao-client` artifact
[source, shell]
----
$ mvn -pl bdm/dao-client -am deploy -DaltDeploymentRepository=<server_id>::<remote_repository_url> <1>
----
...
. Right click on your Business Data Model and select *Export*.
