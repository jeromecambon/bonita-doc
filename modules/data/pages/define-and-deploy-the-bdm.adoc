...
.The generated model dependency template
[source,xml]
----
<dependency>
...
----
<1> In most use cases, when the dependent project is meant to be deployed server side (eg: Rest API Extension, Custom libraries used in process expressions...etc.) the dependency scope of the model should be `provided`. When used and packaged as a client application, `compile` scope (maven default) should be used.
...
It is possible to automate the build and deployment of the `bdm-model` and `bdm-dao-client` artifacts using *only* Maven. Publishing those artifacts can have its purpose if you need to consume them in any other maven or gradle projects outside of a Bonita project.

[CAUTION]
It is not supported for a Bonita project to depend on the `bdm-model` dependency from another Bonita project.

It is required to have https://maven.apache.org/install.html[Maven^] and https://adoptium.net/temurin/releases/?version=11[Java^] installed for the following instructions. 

Open a terminal at the root of the Bonita project.

To install the artifacts in your local repository you can run:
...
NOTE: Only *read operations* can be performed on business objects from a REST API Extension, even with the DAO. Write operations are done through processes.
